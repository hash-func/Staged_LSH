...
void hid_bound_set_1(size_t flame, size_t hash_table_pointer, size_t FP_DB, size_t hash_table, unsigned int flame_index, size_t count_stream_out, size_t locate_stream_out);
...
void __stub____xlnx_cl_hid_bound_set_1(char **argv) {
  ...
  unsigned int flame_index = *((unsigned int*)args[4+1]);
  size_t count_stream_out = *((size_t*)args[5+1]);
  size_t locate_stream_out = *((size_t*)args[6+1]);
 //  pthread_mutex_lock(&__xlnx_cl_hid_bound_set_1_mutex);
  hid_bound_set_1(flame, hash_table_pointer, FP_DB, hash_table, flame_index, count_stream_out, locate_stream_out);
