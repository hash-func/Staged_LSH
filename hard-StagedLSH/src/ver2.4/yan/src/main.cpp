...
//     // int* judge;
//     unsigned int* flame96;
//     // posix_memalign(reinterpret_cast<void**>(&judge), 32, sizeof(int));
...
// 	// Schedule the writing of the inputs(query)
// 	mErr = clEnqueueMigrateMemObjects(mQueue_wait, 1, &mSrcBuf[0], 0, 1, &mEvent_const[0], &req->mEvent[0]);	
...
//         CL_TRUE, CL_MAP_READ, 0, sizeof(int),            1, &req->mEvent[0],  &mEvent_flame[0], &mErr);
//         if (mErr != CL_SUCCESS) {printf("エラー検出\n"); return req;}
//         // judge
//   	    mErr = clSetKernelArg(mKernel_judge,       3, sizeof(cl_mem),       &mDstBuf[0]);
//         /* flame送信 */
//         mErr = clEnqueueMigrateMemObjects(mQueue_wait, 1, &mFlmBuf[0], 0, 1, &mEvent_flame[0], &mEvent_flame[1]);
//         if (mErr != CL_SUCCESS) {printf("エラー検出\n"); break;}
//         /* カーネルの実行 */
//         mErr = clEnqueueTask(mQueue, mKernel_judge, 1,  &mEvent_flame[1], &mEvent_flame[2]);
//         if (mErr != CL_SUCCESS) {printf("エラー検出\n"); break;}
//         mErr = clEnqueueTask(mQueue, mKernel_hid,   1,  &mEvent_flame[1], &mEvent_flame[3]);
//         if (mErr != CL_SUCCESS) {printf("エラー検出\n"); break;}
//         // Schedule the reading of the outputs
//         mErr = clEnqueueMigrateMemObjects(mQueue_wait, 1, &mDstBuf[0], CL_MIGRATE_MEM_OBJECT_HOST, 1, &mEvent_flame[2], &mEvent_flame[4]);
//         if (mErr != CL_SUCCESS) {printf("エラー検出\n"); break;}
//         /* UnMap*/
//         mErr = clEnqueueUnmapMemObject(mQueue_wait, mDstBuf[0], judge,   1, &mEvent_flame[4], &mEvent_flame[5]);
// 	    if (mErr != CL_SUCCESS) {printf("エラー検出\n"); return req;}
//         /* 結果取得,カーネル終了まで待ち */
//         mErr = clWaitForEvents(6, mEvent_flame);
//         if (mErr != CL_SUCCESS) {printf("エラー検出\n"); break;}
//         /* 時間処理 */
//         // 開始時刻
//         mErr = clGetEventProfilingInfo(mEvent_flame[3], CL_PROFILING_COMMAND_START,
//         sizeof(cl_ulong), &start_time, NULL);
//         if (mErr != CL_SUCCESS) {printf("エラー検出\n"); break;}
//         // 終了時刻
//         mErr = clGetEventProfilingInfo(mEvent_flame[2], CL_PROFILING_COMMAND_END,
//         sizeof(cl_ulong), &end_time, NULL);
//         if (mErr != CL_SUCCESS) {printf("エラー検出\n"); break;}
//         // 時間
//         serch_t_temp += end_time - start_time;
...
//         if (mErr != CL_SUCCESS) {printf("エラー検出\n"); break;}
//         mErr = clFinish(mQueue);
//         if (mErr != CL_SUCCESS) {printf("エラー検出\n"); break;}
//         /* 終了条件 */
//         count++;
//         if (*judge >= 0 || j == FLAME_IN_MUSIC-1) {
//             // 戻り値格納
//             *judge_temp = *judge;
//             break;
//         }
...
public:
  ...
  { 
    ...
    // // Map-Buffer
    // int* judge = (int*)clEnqueueMapBuffer(mQueue_wait, mDstBuf[0],
    // CL_TRUE, CL_MAP_READ, 0, sizeof(int),      1, &mEvent_const[1],  &req->mEvent[0], &mErr);
    // if (mErr != CL_SUCCESS) {printf("judge_Mapエラー検出\n"); exit(1);}
    // // judge
  	// mErr = clSetKernelArg(mKernel_judge,       3, sizeof(cl_mem),       &mDstBuf[0]);
    /* flame送信 */
    ...
    *judge_temp = *judge;
	...
	// if (mErr != CL_SUCCESS) {printf("UnMapエラー検出\n"); exit(1);}
    // Register call back to notify of kernel completion
	// clSetEventCallback(req->mEvent[0], CL_COMPLETE, event_cb, &req->mId); 
    // free
    free(flame96);
