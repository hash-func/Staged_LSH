...
// OpenCLホストコードエリア
    ...
    cl::Kernel mKernel_hid      (program, "hid_cal_set_1",      &err);
    cl::Kernel mKernel_bound    (program, "bound_set_1",        &err);
    cl::Kernel mKernel_switch   (program, "switch_set_1",       &err);
    cl::Kernel mKernel_backet   (program, "backet_serch_set_1", &err);
    cl::Kernel mKernel_read4096 (program, "read4096_set_1",     &err);
    cl::Kernel mKernel_hd4096   (program, "hdis4096_cal_set_1", &err);
    cl::Kernel mKernel_out      (program, "out",                &err);
    cl::Kernel mKernel_determin (program, "determin",           &err);
    ...
    cl::Buffer judge_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, 
        sizeof(int),                                    judge_ad,         &err);

    mKernel_hid.setArg      (0, query_buf);
    ...
    mKernel_switch.setArg       (0, FP_DB_buf);
    mKernel_read4096.setArg     (0, FP_DB_buf);
    mKernel_backet.setArg       (0, hash_table_buf);
    mKernel_switch.setArg       (1, hash_table_buf);
    mKernel_bound.setArg        (0, hash_table_pointer_buf);
    mKernel_hid.setArg          (0, query_buf);
    mKernel_hd4096.setArg       (0, query_buf);
    mKernel_out.setArg          (0, judge_buf);
    mKernel_determin.setArg     (0, flag);
    ...
    for (unsigned int i=0; i<QUERY_NUM; i++)
    {
        ...
        // カーネルの実行
        q.enqueueTask(mKernel_hid);
        q.enqueueTask(mKernel_bound);
        q.enqueueTask(mKernel_switch);
        q.enqueueTask(mKernel_backet);
        q.enqueueTask(mKernel_read4096);
        q.enqueueTask(mKernel_hd4096);
        q.enqueueTask(mKernel_out);
        q.enqueueTask(mKernel_determin);

        // 出力のホストメモリへの転送をスケジュール
        // 結果の取得
        q.enqueueMigrateMemObjects({judge_buf}, CL_MIGRATE_MEM_OBJECT_HOST);
