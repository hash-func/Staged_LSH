...
public:
  ...
  { 
    ...
    for (int j=0; j<FLAME_IN_MUSIC; j++) {
        ...
        // flame
        mErr = clSetKernelArg(mKernel_hid,         0, sizeof(cl_mem),       &mFlmBuf[0]);
        if (mErr != CL_SUCCESS) {printf("clSetKernel flameforエラー検出\n"); break;}
        // judge
  	    mErr = clSetKernelArg(mKernel_det,         0, sizeof(cl_mem),       &mDstBuf[0]);
        if (mErr != CL_SUCCESS) {printf("clSetKernel judgeforエラー検出\n"); break;}
        // qe_flag
        mErr = clSetKernelArg(mKernel_det,         2, sizeof(bool),         &qe_flag);
        if (mErr != CL_SUCCESS) {printf("clSetKernel qe_flagforエラー検出\n"); break;}
        /* flame作成 */
        flame96[0] = query[j+0];
        flame96[1] = query[j+1];
        flame96[2] = query[j+2];
        ...
        if (mErr != CL_SUCCESS) {printf("clEnqueueMig flameforエラー検出\n"); break;}
        /* カーネルの実行 */
        mErr = clEnqueueTask(mQueue, mKernel_det,   1,  &mEvent_flame[0], &mEvent_flame[1]);
        if (mErr != CL_SUCCESS) {printf("clEnqueueTask detforエラー検出\n"); break;}
        mErr = clEnqueueTask(mQueue, mKernel_hid,   1,  &mEvent_flame[0], &mEvent_flame[2]);
        if (mErr != CL_SUCCESS) {printf("clEnqueueTask hidforエラー検出\n"); break;}
        ...
        for (int e = 0; e<n; e++) {
            ...
            if (mErr != CL_SUCCESS) {printf("clReleaseEventエラー検出\n"); break;}
        }
        mErr = clFinish(mQueue_wait);
        if (mErr != CL_SUCCESS) {printf("clFinish waitエラー検出\n"); break;}
        /* 終了条件 */
        if (*judge >= 0) break;
    }
    // 戻り値格納
    *judge_temp = *judge;
    ...
    if (mErr != CL_SUCCESS) {printf("clUnmap flameエラー検出\n"); return req;}
    mErr = clEnqueueUnmapMemObject(mQueue_wait, mDstBuf[0], judge,   1, &req->mEvent[4], &req->mEvent[5]);
	if (mErr != CL_SUCCESS) {printf("clUnmap judgeエラー検出\n"); return req;}
