...
)
{
    ...
    #pragma HLS PIPELINE
        reg = hd_cal32_6((ap_uint<32>) query[i], (ap_uint<32>) FP_DB[db_point+i]);
        haming_dis += reg;
    }
...
)
{
    ...
    /* 4096bit Haming距離計算 */
    haming_dis_seisa = fpdb_locate_6(
        query,              // クエリ
        FP_DB,              // FPデータベース
        db_point            // DB中楽曲開始位置
    );
...
{
#pragma HLS TOP name=table_serch_6
...
bool in_flag = true;
    ...
    #pragma HLS loop_tripcount min=1 max=126    // 反復回数の合計を手動指定
        ...
        /* Serch Module */
        hash_value = hid_cal_6(
            ((tempA32, tempB32), tempC32)
                            // フレーム
        );

        /* HT_SERACH */
        music_index = backet_serch_6(
            hash_value,         // ハッシュ値
            hash_table,         // ハッシュテーブル
