...
#pragma HLS INTERFACE m_axi depth=32768 port=hash_table_pointer bundle=pointer_hid_set_1
    ...
    hash_gen_loop: for (flame_index=0; flame_index<FLAME_IN_MUSIC; flame_index++)
    {
        ...
        //     if ((ap_uint<1>)complete_stream_det.data == (ap_uint<1>) 1) {break;}
        // }
        /* Stream-portへ送信 */
    ...
    if (flame_index == FLAME_IN_MUSIC) {
        /* 続行信号受け取り */
        // printf("hid_bound : 終了信号待ち\n");
        complete_stream_det = complete_stream_in.read();
        // printf("hid_bound : 終了信号受信後\n");
        /*0でも1でも終了処理には変わらない*/
    }
    /* 最終処理なら全てのカーネル停止 */
    if (!flag) {
        complete_out.data = 1;
        complete_stream_out1.write(complete_out);
        complete_stream_out2.write(complete_out);
        complete_stream_out3.write(complete_out);
...
//         complete_stream_out3.write(complete_stream_out);
//         printf("his_bound : カーネル停止信号送信\n");
//     }
// }
// }
...
//             tempB32 = tempC32;
//             flame_index++;
//         if (flame_index == FLAME_IN_MUSIC) break;
//     }
//     /* 最後の処理の判定待ち */
//     if (flame_index == FLAME_IN_MUSIC) {
//         /* 続行信号受け取り */
