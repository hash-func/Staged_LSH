...
public:
    ...
    const unsigned int division_num )	
  {
    ...
    mKernel_query       = clCreateKernel(Program, "query_set_1",        &mErr);
    ...
    // hash_table_pointer
    clSetKernelArg(mKernel_hid,         0, sizeof(cl_mem),       &mConstBuf[2]);
    // flag -> hd96
    bool flag_hd96 = true;
  ...
  { 
    ...
    sizeof(int), judge_temp, &mErr);
  	...
  	// Set the kernel arguments
    // query
  	clSetKernelArg(mKernel_query,       0, sizeof(cl_mem),       &mSrcBuf[0]);
    clSetKernelArg(mKernel_hd4096,      0, sizeof(cl_mem),       &mSrcBuf[0]);
    // flag
    clSetKernelArg(mKernel_query,       1, sizeof(bool),         &flag);
    // judge
	...
	clEnqueueTask(mQueue, mKernel_query,    1,  &req->mEvent[0], &req->mEvent[1]);
