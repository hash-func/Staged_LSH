...
// 	mKernel_1  = clCreateKernel(Program, "table_serch", &mErr);
...
//   	clSetKernelArg(mKernel_1, 5, sizeof(cl_mem),       &mDstBuf[0]);
//     clSetKernelArg(mKernel_1, 4, sizeof(int),          &hash_index);
...
//         /* 検索処理（FPGA） */
//         request[0] = Serch(query, judge_ad, 1);
...
// OpenCLホストコードエリア
    ...
    // カーネルの作成
    cl::Kernel mKernel      (program, "table_serch",    &err);
    // イベント
    // 常用カーネル
    ...
    mKernel.setArg      (3,  FP_DB_buf);
    // hash_table
    mKernel.setArg      (2,  hash_table_buf);
    mKernel.setArg      (4,  hash_table_buf);
    // hash_table_pointer
    mKernel.setArg      (5,  hash_table_pointer_buf);
    // query
    mKernel.setArg      (0,  query_buf);
    // judge
    mKernel.setArg      (7,  judge_buf);
