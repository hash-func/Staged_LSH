...
sim_embedded_scheduler_sw_imp::sim_embedded_scheduler_sw_imp(const sc_core::sc_module_name& name):sc_module(name),
...
void sim_embedded_scheduler_sw_imp::threadInterruptHandler() {
	...
	wait(maxi_lite_mb_aclk.posedge_event());
		if(this->isInterruptEnable() && irq.read()==1)  ert::cu_interrupt_handler();
	}
}
