...
u32 XTable_serch_Get_return(XTable_serch *InstancePtr) {
    ...
    Data = XTable_serch_ReadReg(InstancePtr->Control_BaseAddress, XTABLE_SERCH_CONTROL_ADDR_AP_RETURN);
    return Data;
}
...
void XTable_serch_Set_bit_element(XTable_serch *InstancePtr, u64 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XTable_serch_WriteReg(InstancePtr->Control_BaseAddress, XTABLE_SERCH_CONTROL_ADDR_BIT_ELEMENT_DATA, (u32)(Data));
    XTable_serch_WriteReg(InstancePtr->Control_BaseAddress, XTABLE_SERCH_CONTROL_ADDR_BIT_ELEMENT_DATA + 4, (u32)(Data >> 32));
}

u64 XTable_serch_Get_bit_element(XTable_serch *InstancePtr) {
    u64 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XTable_serch_ReadReg(InstancePtr->Control_BaseAddress, XTABLE_SERCH_CONTROL_ADDR_BIT_ELEMENT_DATA);
    Data += (u64)XTable_serch_ReadReg(InstancePtr->Control_BaseAddress, XTABLE_SERCH_CONTROL_ADDR_BIT_ELEMENT_DATA + 4) << 32;
    return Data;
}
